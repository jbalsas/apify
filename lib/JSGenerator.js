...
(function () {
    ...
    function generateDocsForSourceTree(src, output, excludes) {
        ...
        // TODO Unfold a bit, and improve exclude regexp detection and configuration
        find.file(/\.js$/, src, function (all) {
            modules = all.filter(function (module) {
                return excludes.every(function (rule) {
                    return !(new RegExp(rule).test(module));
                });
            }).map(function (module) {
                var data = pathRegexp.exec(module);
                return {
                    fullPath: data[0],
                    path: data[1],
                    name: data[2]
            ...
            modules = _.sortBy(modules, function (module) {
                return module.path.toLowerCase();
            });
                        
            orderedModules = _.map(_.groupBy(modules, function (module) {
                return module.path;
            ...
            modules.forEach(function (mod) {
                parseModule(mod.fullPath, function (module) {
                    ...
                    module.otherModules = orderedModules;
                    module.otherModulesCount = modules.length;
                    generateModulePage(module, output);
                });
            });
            
            generateSearchIndex(modules, output);
            
            // Generates the docs index
            generateIndexPage({
                otherModules: orderedModules,
