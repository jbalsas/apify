...
(function () {
     ...
     *
     */
    function generateSearchIndex(modules, output) {
    ...
    function generateModulePage(module, output) {
        ...
        fs2.mkdir(moduleFolder, {intermediate: true}, function () {
     ...
     */
    function generateDocsForSourceTree(src, output, workspace) {
                    
        // Generates documentation for each of the found modules
        workspace.modules.forEach(function (mod) {
            parseModule(mod.fullPath, function (module) {
                module.name = mod.name;
                module.path = mod.path.replace(src, "");
                module.otherModules = workspace.groups;
                module.otherModulesCount = workspace.length;
                generateModulePage(module, output);
            });
        });
        
        generateSearchIndex(workspace.modules, output);
        
        generateIndexPage({
            otherModules: workspace.groups,
            otherModulesCount: workspace.length
        }, output);
    }
    ...
    function init(config) {
        ...
        workspace.load(source, includes, excludes).then(function (workspace) {
            clearOutputDir(output).then(function () {
                ncp("assets", path.join(output, "assets"), function () {
                    generateDocsForSourceTree(source, output, workspace);
                });
            }, function (error) {
                console.error(error);
            }).end();
