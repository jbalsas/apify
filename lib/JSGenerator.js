...
(function () {
    ...
    function generateSearchIndex(modules, output) {
        var modulesIndex = _.map(modules, function (module) {
            return { name: module[1], value: module[1] };
        });
        
        fs2.mkdir(path.join(output, "assets/data"), {intermediate: true}).then(function () {
            fs.writeFileSync(path.join(output, "assets/data/modules.json"), JSON.stringify(modulesIndex), "UTF-8");
        });
    }
