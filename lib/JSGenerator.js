...
(function () {
     ...
     */
    function generateDocsForSourceTree(output, fileset) {
        
        // Generates documentation for each of the found modules
        fileset.files.forEach(function (mod) {
            parseModule(mod.fullPath, function (module) {
                module.name = mod.name;
                module.path = mod.docPath;
                module.otherModules = fileset.groups;
                module.otherModulesCount = fileset.length;
                generateModulePage(module, output);
            });
        });
        
        generateSearchIndex(fileset.files, output);
        
        generateIndexPage({
            otherModules: fileset.groups,
            otherModulesCount: fileset.length
        }, output);
    }
    ...
    function init(config) {
        ...
        workspace.load(source, includes, excludes).then(function (fileset) {
            workspace.createOutputSkeleton(output, fileset).then(function (fileset) {
                generateDocsForSourceTree(output, fileset);
            });
        });
    }
