 ...
 * Parses the file data using dox, and then runs the output through the different
...
(function () {
    ...
    ModuleParser.prototype.parse = function (url) {
        ...
        fq.readFile(url, 'UTF-8', function (err, data) {
            ...
            module.hasDependencies = module.dependencies.length ? '' : 'hide';
            
            module.variables = _sortMembers(module.variables);
            module.hasVariables = module.variables.length ? '' : 'hide';
            
            module.functions = _sortMembers(module.functions);
            module.hasFunctions = module.functions.length ? '' : 'hide';
            
            module.classes = _.toArray(module.classes).map(function (clazz) {
                clazz.properties = _sortMembers(clazz.properties);
                clazz.hasProperties = clazz.properties.length ? '' : 'hide';
                
                clazz.methods = _sortMembers(clazz.methods);
                clazz.hasMethods = clazz.methods.length ? '' : 'hide';
                
                return clazz;
            });
            
            module.hasClasses = module.classes.length ? '' : 'hide';
