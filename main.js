...
(function () {
    ...
    function init(src, output) {
        ...
        Object.keys(args).forEach(function (index) {
            if (args[index].startsWith("--exclude=")) {
                excludes = args[index].substr("--exclude=".length).split(",");
            }
        });
    ...
    // TODO add comman line power to better define the input parameters
    // https://npmjs.org/package/commander
    // https://npmjs.org/package/node-commandline
    // https://npmjs.org/package/OptionParser
    // https://npmjs.org/package/argparser
    
    // Drop two first arguments from command line "node" and "main.js"
    init.apply(this, process.argv.slice(2));
